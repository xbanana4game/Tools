...
:ArchiveKF
	...
	SET UPDATE_7Z_FILE=%BACKUPS_DIR%\%TARGET%_-_Update-%yyyy%%mm%%dd%.7z
	CALL :TREE_KF_SERVER %TARGET%
	CALL :CheckDirectory "%KILLINGFLOOR_DIR%\KF-SERVER\%TARGET%"
	IF NOT EXIST "%BACKUPS_DIR%\%TARGET%.7z" (
		7z a -t7z "%BACKUPS_DIR%\%TARGET%.7z" "%KILLINGFLOOR_DIR%\KF-SERVER\%TARGET%" -mx=9
		7z l "%BACKUPS_DIR%\%TARGET%.7z">"%BACKUPS_DIR%\%TARGET%.7z.txt"
	) ELSE (
		7z u "%BACKUPS_DIR%\%TARGET%.7z" -u- -u%UPDATE_SWITCH_OPT%!%UPDATE_7Z_FILE% "%KILLINGFLOOR_DIR%\KF-SERVER\%TARGET%"
		7z l "%UPDATE_7Z_FILE%">"%UPDATE_7Z_FILE%"
	)
	EXIT /B %ERRORLEVEL%
