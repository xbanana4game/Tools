...
:MakeRobocopyList
	...
	FOR /F "tokens=1,2 delims= " %%C IN (%ARCHIVE_PROFILE_FILE%) DO (
		...
		IF EXIST "%ARCHIVE_ROOT_DIR_NAME%\%%C" (
			ECHO !PROFILE:\=_!,%ARCHIVE_PATH%%ARCHIVE_PROFILE%\%%C,%COPY_TO_DRIVE%:\_work\%ARCHIVE_PROFILE%>>COPY_LIST.txt
		)
	)
	IF NOT EXIST %COPY_TO_DRIVE%:\_work MD %COPY_TO_DRIVE%:\_work
	COPY COPY_LIST.txt %COPY_TO_DRIVE%:\_work\COPY_LIST.txt
	DEL COPY_LIST.txt
	COPY %TOOLS_DIR%\Archive-Tools\ROBOCOPY_FILES.cmd %COPY_TO_DRIVE%:\_work\ROBOCOPY_FILES.cmd
	EXIT /B 0
