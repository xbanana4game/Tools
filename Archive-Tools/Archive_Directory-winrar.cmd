...
REM SET ARCHIVE_OUTPUT_DIR=%DRIVE_LETTER%\_work
...
IF NOT DEFINED WINRAR_LOG SET WINRAR_LOG=winrar-%yyyy%%mm%%dd%%hh%%mn%%ss%.log
REM IF NOT DEFINED WINRAR_LOG_OPTIONS SET WINRAR_LOG_OPTIONS=-ilog%WINRAR_LOG% 

IF NOT "%Z_PASSWORD%"=="NONE" SET ARCHIVE_OPT_PW=-hp%Z_PASSWORD%

FOR %%i in (%*) DO (
	ECHO %%i
	winrar a %SDEL_OPT% %ARCHIVE_OPT_PW% -rr3p -rv%RECOVERY_VOLUME_NUMBER% %OPT_VOLUME% %WINRAR_LOG_OPTIONS% -m%OPT_COMPRESSION_LEVEL% "%ARCHIVE_OUTPUT_DIR%\%%~nxi.rar" "%%i"
)
PAUSE
EXIT
