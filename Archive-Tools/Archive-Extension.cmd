...
:EXECUTE_ARCHIVE_EXT
	...
	ECHO 7z a -tzip %SDEL_OPT% %OUTPUT_DIR_NAME%\%OUTPUT_FILENAME%.zip  %ARCHIVE_PREFIX%*.%ARCHIVE_EXT% -r -xr!Archive-Extension.cmd -xr!%OUTPUT_FILENAME%.zip -xr!%OUTPUT_FILENAME%.zip.txt -mx=0
...
:EXECUTE_ARCHIVE_EXT_SETTINGS_FROM_FILE
	...
	FOR /F "skip=1 tokens=1,2,3 delims=;" %%C IN (%EXT_LIST_FILE%) DO (
		...
		IF %%C==1 (
			...
			ECHO 7z a -tzip %SDEL_OPT% %OUTPUT_DIR%\%%D %%E -r -xr!Archive-Extension.cmd -x!%OUTPUT_DIR_NAME% -xr!%%D -xr!%%D.txt -xr!.ts -mx=0
