...
:MOVE_VIDEOS
	...
	SET MP3TAG_M3U8_FILE=%VIDEOS_ARCHIVE_DIR%\%TARGET_DIR%\%TARGET_DIR%_%yyyy%-%mm%-%dd%.m3u8
	SET STORE_M3U8_FILE=%VIDEOS_ARCHIVE_DIR%\%TARGET_DIR%\%TARGET_DIR%_%yyyy%-%mm%-%dd%_%hh%%mn%%ss%.m3u8
	ECHO Playlist file name is %MP3TAG_M3U8_FILE%
	IF EXIST %VIDEOS_ARCHIVE_DIR%\%TARGET_DIR% Mp3tag.exe /fp:"%VIDEOS_ARCHIVE_DIR%\%TARGET_DIR%"
	IF EXIST %MP3TAG_M3U8_FILE% (
		ECHO Execute macro m3u2wpl.ppa. alt+q
		MOVE %MP3TAG_M3U8_FILE% %STORE_M3U8_FILE%
		%SAKURA% %STORE_M3U8_FILE%
		MD %VIDEOS_ARCHIVE_DIR%\%TARGET_DIR%\__playlist
	)
