...
:MOVE_VHDX
	...
	IF 1 EQU %ARCHIVE_FLG% (
		MD %VHDX_STORE_DIR%
		IF EXIST %DOWNLOADED_VIDEOS_DIR%\*.vhdx MOVE %DOWNLOADED_VIDEOS_DIR%\*.vhdx %VHDX_STORE_DIR%
...
:MOVE_VIDEOS
	...
	IF 1 EQU %ARCHIVE_FLG% (
		...
		IF EXIST %DOWNLOADED_VIDEOS_DIR%\%TARGET_DIR% MOVE %DOWNLOADED_VIDEOS_DIR%\%TARGET_DIR% %VIDEOS_STORE_DIR%\VD_%yyyy%%mm%%dd%\
...
:COPY_VIDEOS
	...
	IF 1 EQU %ARCHIVE_FLG% (
		CHOICE /C YN /T 5 /D Y /M "COPY VIDEOS %VIDEOS_STORE_DIR%\VD_%yyyy%%mm%%dd%? -> %XCOPY_DIRECTORY_VIDEOS%"
		IF %ERRORLEVEL% EQU 2 (EXIT /B)
