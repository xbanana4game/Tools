...
REM SET XXX_VIDEOS_DIR=%DOWNLOADED_VIDEOS_DIR%\xxx
REM SET VIDEO_DB_FILE_DEFAULT=%CONFIG_DIR%\videos_%USERDOMAIN%.sqlite3
REM SET VIDEO_DB_FILE_XXX=%CONFIG_DIR%\videos_xxx.sqlite3
REM SET VIDEO_STORE_NAME=VD_%yyyy%%mm%%dd%-%hh%%mn%
REM SET VIDEOS_STORE_DIR=%STORE_DIR%\%VIDEO_STORE_NAME%
...
IF NOT DEFINED VIDEO_STORE_NAME SET VIDEO_STORE_NAME=VD_%yyyy%%mm%%dd%-%hh%%mn%
IF NOT DEFINED VIDEOS_STORE_DIR SET VIDEOS_STORE_DIR=%STORE_DIR%\%VIDEO_STORE_NAME%
IF NOT DEFINED VIDEOS_ARCHIVE_DIR SET VIDEOS_ARCHIVE_DIR=%ARCHIVE_DIR%\%VIDEO_STORE_NAME%
IF NOT DEFINED DOWNLOADED_VIDEOS_DIR SET DOWNLOADED_VIDEOS_DIR=%DOWNLOADS_DIR%
IF NOT DEFINED XXX_VIDEOS_DIR SET XXX_VIDEOS_DIR=%DOWNLOADED_VIDEOS_DIR%\xxx
...
IF NOT DEFINED VIDEO_DB_FILE_DEFAULT SET VIDEO_DB_FILE_DEFAULT=%CONFIG_DIR%\videos_%USERDOMAIN%.sqlite3
IF NOT DEFINED VIDEO_DB_FILE_XXX  SET VIDEO_DB_FILE_XXX=%CONFIG_DIR%\videos_xxx.sqlite3
...
:MOVE_VIDEOS
	SET TARGET_DIR=%1
	ECHO MOVE %TARGET_DIR% %VIDEOS_ARCHIVE_DIR%
	MD %VIDEOS_ARCHIVE_DIR%
	IF EXIST %DOWNLOADED_VIDEOS_DIR%\%TARGET_DIR% MOVE %DOWNLOADED_VIDEOS_DIR%\%TARGET_DIR% %VIDEOS_ARCHIVE_DIR%\
	IF NOT EXIST %VIDEOS_ARCHIVE_DIR%\%TARGET_DIR% (EXIT /B)
	RMDIR %VIDEOS_ARCHIVE_DIR%
	REM SET MP3TAG_M3U8_FILE=%VIDEOS_ARCHIVE_DIR%\%TARGET_DIR%\%TARGET_DIR%_%yyyy%-%mm%-%dd%.m3u8
	REM SET STORE_M3U8_FILE=%VIDEOS_ARCHIVE_DIR%\%TARGET_DIR%\%TARGET_DIR%_%yyyy%-%mm%-%dd%_%hh%%mn%%ss%.m3u8
	REM ECHO Playlist file name is %MP3TAG_M3U8_FILE%
	REM IF EXIST %VIDEOS_ARCHIVE_DIR%\%TARGET_DIR% Mp3tag.exe /fp:"%VIDEOS_ARCHIVE_DIR%\%TARGET_DIR%"
	REM IF EXIST %MP3TAG_M3U8_FILE% (
		REM ECHO Execute macro m3u2wpl.ppa. alt+q
		REM MOVE %MP3TAG_M3U8_FILE% %STORE_M3U8_FILE%
		REM %SAKURA% %STORE_M3U8_FILE%
		REM MD %VIDEOS_ARCHIVE_DIR%\%TARGET_DIR%\__playlist
	REM )
	SET REMOVE_PATH_STR=%VIDEOS_ARCHIVE_DIR%\
	SET VIDEO_DB_FILE=%VIDEO_DB_FILE_DEFAULT%
	CALL :EXEC_UPDATE_VIDEO_DB %VIDEOS_ARCHIVE_DIR%\%TARGET_DIR%
	EXIT /B

:COPY_VIDEOS
	IF NOT DEFINED XCOPY_DIRECTORY_VIDEOS EXIT /B
	RMDIR %VIDEOS_ARCHIVE_DIR%
	IF NOT EXIST %VIDEOS_ARCHIVE_DIR% EXIT /B
	SET MP3TAG_M3U8_FILE=%VIDEOS_ARCHIVE_DIR%\%VIDEO_STORE_NAME%_%yyyy%-%mm%-%dd%.m3u8
	SET VIDEOS_M3U8_FILE=%VIDEOS_ARCHIVE_DIR%\%VIDEO_STORE_NAME%.m3u8
	IF EXIST %VIDEOS_ARCHIVE_DIR% Mp3tag.exe /fp:"%VIDEOS_ARCHIVE_DIR%"
	IF EXIST %MP3TAG_M3U8_FILE% (
		REM TYPE %MP3TAG_M3U8_FILE%>>%CONFIG_DIR%\yt-dlp-history\VD_%yyyy%%mm%.m3u8
		MOVE %MP3TAG_M3U8_FILE% %VIDEOS_M3U8_FILE%
		%SAKURA% %VIDEOS_M3U8_FILE%
	)
	CHOICE /C YN /T 5 /D Y /M "COPY VIDEOS %VIDEOS_ARCHIVE_DIR%? -> %XCOPY_DIRECTORY_VIDEOS%"
	IF %ERRORLEVEL% EQU 2 (EXIT /B)
	ROBOCOPY %VIDEOS_ARCHIVE_DIR% %XCOPY_DIRECTORY_VIDEOS% %ROBOCOPY_COPY_OPTIONS% %ROBOCOPY_LOG_OPTIONS%
	IF ERRORLEVEL 8 (
		REM ERROR
		MOVE %VIDEOS_ARCHIVE_DIR%\* %DOWNLOADED_VIDEOS_DIR%
		RMDIR %VIDEOS_ARCHIVE_DIR%
		NOTEPAD %ROBOCOPY_LOG%
	) ELSE (
		REM SUCCESS
		REM EXPLORER %XCOPY_DIRECTORY_VIDEOS%
		MOVE %VIDEOS_ARCHIVE_DIR% %VIDEOS_STORE_DIR%
		DEL %ROBOCOPY_LOG%
	)
...
:F_XCOPY_XXX
	...
	IF NOT EXIST %XXX_VIDEOS_DIR% EXIT /B
	Mp3tag.exe /fp:"%XXX_VIDEOS_DIR%"
	SET REMOVE_PATH_STR=%XXX_VIDEOS_DIR%\
	SET VIDEO_DB_FILE=%VIDEO_DB_FILE_DEFAULT%
	IF DEFINED VIDEO_DB_FILE_XXX SET VIDEO_DB_FILE=%VIDEO_DB_FILE_XXX%
	REM SET FORCE_UPDATE_FLG=1
	CALL :EXEC_UPDATE_VIDEO_DB %XXX_VIDEOS_DIR%
	REM SET FORCE_UPDATE_FLG=
	IF NOT DEFINED ROBOCOPY_TARGET_XXX_FILE SET ROBOCOPY_TARGET_XXX_FILE=*5star* *4star*
	IF NOT DEFINED ROBOCOPY_EXTRA_OPTIONS SET ROBOCOPY_EXTRA_OPTIONS=/XD *-low
	ROBOCOPY %XXX_VIDEOS_DIR% %XCOPY_DIRECTORY_XXX% %ROBOCOPY_TARGET_XXX_FILE% %ROBOCOPY_EXTRA_OPTIONS% %ROBOCOPY_COPY_OPTIONS% %ROBOCOPY_LOG_OPTIONS%
	IF ERRORLEVEL 8 (
		NOTEPAD %ROBOCOPY_LOG%
	) ELSE (
...
:F_XCOPY_DIR
	...
	SET REMOVE_PATH_STR=%DOWNLOADS_DIR%\
	SET VIDEO_DB_FILE=%VIDEO_DB_FILE_DEFAULT%
	CALL :IS_EXIST_MP4 %DOWNLOADS_DIR%\%DIR_NAME%
	IF %ERRORLEVEL% EQU 0 (
		Mp3tag.exe /fp:"%DOWNLOADS_DIR%\%DIR_NAME%"
		CALL :EXEC_UPDATE_VIDEO_DB %DOWNLOADS_DIR%\%DIR_NAME%
	)
...
:EXEC_UPDATE_VIDEO_DB
	SETLOCAL
	SET TARGET_DIR=%~1
	IF NOT DEFINED REMOVE_PATH_STR SET REMOVE_PATH_STR=%TARGET_DIR%\
	IF DEFINED SKIP_URL_FLG SET UPDATE_VIDEO_DB_OPT=--skip-url %UPDATE_VIDEO_DB_OPT%
	IF DEFINED FORCE_UPDATE_FLG SET UPDATE_VIDEO_DB_OPT=--force-update %UPDATE_VIDEO_DB_OPT%
	IF DEFINED INSERT_ONLY_FLG SET UPDATE_VIDEO_DB_OPT=--insert-only %UPDATE_VIDEO_DB_OPT%
	IF DEFINED CHANNEL_DB_PATH SET UPDATE_VIDEO_DB_OPT=--channels-db "%CHANNEL_DB_PATH%" %UPDATE_VIDEO_DB_OPT%
	IF DEFINED VIDEO_DB_FILE SET UPDATE_VIDEO_DB_OPT=--videos-db "%VIDEO_DB_FILE%" %UPDATE_VIDEO_DB_OPT%
