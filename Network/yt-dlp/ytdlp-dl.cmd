...
REM
REM ======================================================================
SET YTDLP_CONFIG_TEMPLATE_DIR=.
IF NOT "%1"=="" (
	SET YTDLP_PROFILE_FILE=%~1
	SET YTDLP_PROFILE=%~n1
) ELSE (
	FOR %%i IN (%YTDLP_CONFIG_TEMPLATE_DIR%\*.dlp) DO (
		ECHO %%~ni
	)
	SET /P YTDLP_PROFILE="->"
	SET YTDLP_PROFILE_FILE=%YTDLP_CONFIG_TEMPLATE_DIR%\!YTDLP_PROFILE!.dlp
)
...
REM IF NOT DEFINED YTDLP_DL_LIST_FILE_EXP SET YTDLP_DL_LIST_FILE_EXP=%YTDLP_PROFILE%.*.%END_FILE_EXT%
IF NOT DEFINED INPUT_DL_LIST_FILE_DEFAULT SET INPUT_DL_LIST_FILE_DEFAULT=%YTDLP_CONF_DIR%\%YTDLP_PROFILE%.txt
IF NOT DEFINED INPUT_DL_LIST_FILE SET INPUT_DL_LIST_FILE=%INPUT_DL_LIST_FILE_DEFAULT%
...
IF NOT DEFINED YTDLP_CONFIG_FILE SET YTDLP_CONFIG_FILE=%DLURL_LIST%.conf
...
IF NOT DEFINED BATCH_FILE SET BATCH_FILE=%YTDLP_CONF_DIR%\__ytdlp_%yyyy%%mm%%dd%_%YTDLP_DOMAIN%.cmd
...
:SET_INPUT_MODE
FINDSTR /C:"#MODE_INPUT" %YTDLP_PROFILE_FILE%
IF NOT ERRORLEVEL 1 (
	SET SELECTED_MODE=1
)
FINDSTR /C:"#MODE_BATCH" %YTDLP_PROFILE_FILE%
IF NOT ERRORLEVEL 1 (
	SET SELECTED_MODE=3
)

REM FOR %%i in (%YTDLP_DL_LIST_FILE_EXP%) DO (
	REM SET INPUT_DL_LIST_FILE=%%i
REM )
IF NOT DEFINED SELECTED_MODE (
	CHOICE /C 123456 /M "#MODE_ 1:INPUT 2:FILE 3:BATCH 4:PLAYLIST 5:FORMAT 6:UPDATE"
	SET SELECTED_MODE=!ERRORLEVEL!
)
IF %SELECTED_MODE% EQU 1 (
	SET MODE=INPUT
) ELSE IF %SELECTED_MODE% EQU 2 (
	SET MODE=FILE
...
) ELSE IF %SELECTED_MODE% EQU 3 (
	SET MODE=FILE
	SET BATCH_ONLY=1
	SET DLURL_LIST_NAME=%DLURL_LIST_NAME%_batch
	SET DLURL_LIST_NAME=!DLURL_LIST_NAME:.=_!
	SET DLURL_LIST=%DLURL_LIST%_batch
	SET DLURL_LIST=!DLURL_LIST:.=_!
	ECHO DEFAULT: %INPUT_DL_LIST_FILE%
	FOR %%i in (%DESKTOP_DIR%\*.txt) DO ECHO %%~fi
	IF NOT EXIST %INPUT_DL_LIST_FILE% SET /P INPUT_DL_LIST_FILE="input txt: "
) ELSE IF %SELECTED_MODE% EQU 4 (
	SET MODE=PLAYLIST
) ELSE IF %SELECTED_MODE% EQU 5 (
	SET MODE=FORMAT
	GOTO :CHECK_FORMAT
) ELSE IF %SELECTED_MODE% EQU 6 (
	yt-dlp.exe -vU
...
CALL :SET_YTDLP_CONFIG_FILE
ECHO YTDLP_CONF_OPT: %YTDLP_CONF_OPT%
...
IF "%FILE_EXT%"=="csv" (
	...
	SET YTDLP_DLURL_LIST_NAME=%DLURL_LIST_NAME%.csv.txt
) ELSE (
...
IF %INPUT_DL_LIST_FILE_DEFAULT%==%INPUT_DL_LIST_FILE% DEL %INPUT_DL_LIST_FILE_DEFAULT%
...
REM COPY %YTDLP_DLURL_LIST% %YTDLP_LOCK_FILE%
CALL :FIND_HISTORY %DLURL_LIST% %YTDLP_DLURL_LIST%
...
SET DLURL_LIST=%~2
SET YTDLP_DLURL_LIST=%~2.%END_FILE_EXT%
IF NOT EXIST %YTDLP_CONFIG_FILE% EXIT /B 1
IF NOT EXIST %DLURL_LIST% EXIT /B 1
...
IF EXIST %YTDLP_ERR_FILE% (
	...
	IF ERRORLEVEL 1 (
		...
		IF NOT EXIST %YT_DLP_LOG_DIR% MD %YT_DLP_LOG_DIR%
		COPY %DESKTOP_DIR%\%DLURL_LIST_NAME%.err %DLURL_LIST_DIR%\
...
DEL %DLURL_LIST_DIR%\%DLURL_LIST_NAME%*.list
...
IF "%BATCH_ONLY%"=="1" (
	SET YTDLP_CONFIG_FILE_BATCH=%DLURL_LIST%.conf
	MOVE %YTDLP_CONFIG_FILE% !YTDLP_CONFIG_FILE_BATCH!
	SET YTDLP_CONFIG_FILE=!YTDLP_CONFIG_FILE_BATCH!
)
SET YTDLP_CONF_OPT=--config-location %YTDLP_CONFIG_FILE%
EXIT /B
...
:SET_SKIP_MODE
FINDSTR /C:"#SKIP_MODE_OFF" %YTDLP_PROFILE_FILE%
IF ERRORLEVEL 1 (
	REM PASS
) ELSE (
	SET SKIP_FLG=0
	SET DLURL_HISTORY_FILES=%DLURL_HISTORY_DIR%\_*.log
	EXIT /B
)

FINDSTR /C:"#SKIP_MODE_ON_PREFIX" %YTDLP_PROFILE_FILE%
IF ERRORLEVEL 1 (
	REM PASS
) ELSE (
	SET SKIP_FLG=1
	IF NOT DEFINED YTDLP_DOMAIN SET /P YTDLP_DOMAIN="YTDLP_DOMAIN: "
	SET DLURL_HISTORY_FILES=%DLURL_HISTORY_DIR%\_!YTDLP_DOMAIN!-add*.log
	SET SKIP_QUALITY=%YTDLP_PROFILE%
	ECHO DLURL_HISTORY_FILES: !DLURL_HISTORY_FILES!
	EXIT /B
)
FINDSTR /C:"#SKIP_MODE_ON" %YTDLP_PROFILE_FILE%
IF ERRORLEVEL 1 (
	REM PASS
) ELSE (
	SET SKIP_FLG=1
...
:SET_RESOLUTION
FINDSTR /C:"#res:RESOLUTION" %YTDLP_PROFILE_FILE%
IF ERRORLEVEL 1 (
	REM PASS
) ELSE (
...
SET OUTPUT_DLURL_LIST_PATH=%2
DEL %OUTPUT_DLURL_LIST_PATH%
IF NOT DEFINED INPUT_DLURL_LIST_PATH EXIT 1
IF NOT DEFINED OUTPUT_DLURL_LIST_PATH EXIT 1
SET STRIP_URL_LIST_FILE=%VIDEOS_DIR%\strip_url_%yyyy%-%mm%-%dd%_T%hh%%mn%%ss%.txt
IF NOT DEFINED DLURL_RESERVED_FILE (
	...
	IF EXIST %OUTPUT_DLURL_LIST_PATH% DEL %OUTPUT_DLURL_LIST_PATH%
)
SET DLURL_HISTORY_FILES=%DLURL_HISTORY_FILES% %DLURL_RESERVED_FILE%
ECHO strip_url.py %INPUT_DLURL_LIST_PATH% %STRIP_URL_LIST_FILE%
strip_url.py %INPUT_DLURL_LIST_PATH% %STRIP_URL_LIST_FILE%>>%YTDLP_LOCK_FILE%
IF ERRORLEVEL 1 (
	ECHO ERROR: strip_url.py Fail. strip_url.py %INPUT_DLURL_LIST_PATH% %STRIP_URL_LIST_FILE%>>%YTDLP_ERR_FILE%
	EXIT 1
)
MOVE %STRIP_URL_LIST_FILE% %OUTPUT_DLURL_LIST_PATH%
REM ECHO FINDSTR /C:"URL" %DLURL_HISTORY_FILES%
REM FOR /F "tokens=1,2 delims=," %%i IN (%STRIP_URL_LIST_FILE%) DO (
	REM FINDSTR /C:"%%j" %DLURL_HISTORY_FILES%>nul 2>&1
	REM IF ERRORLEVEL 1 (
		REM ECHO DL:%%i
		REM ECHO %%i>>%OUTPUT_DLURL_LIST_PATH%
	REM ) ELSE (
		REM ECHO Skip:%%i
		REM ECHO #Skip:%%i>>%YTDLP_CONFIG_FILE%
	REM )
REM )
IF NOT EXIST %OUTPUT_DLURL_LIST_PATH% TYPE nul>%OUTPUT_DLURL_LIST_PATH%
...
IF NOT EXIST %BATCH_FILE% (
	...
	ECHO IF NOT DEFINED BATCH_LIST_DIR SET BATCH_LIST_DIR=%YTDLP_CONF_DIR%>>%BATCH_FILE%
	ECHO REM SET YTDLP_BATCH_OPT=>>%BATCH_FILE%
	ECHO;>>%BATCH_FILE%
)
ECHO :%DLURL_LIST_NAME%_START>>%BATCH_FILE%
ECHO TITLE %%MERGED_BATCH_NAME%% - %DLURL_LIST_NAME%>>%BATCH_FILE%
...
SET CONFIG_FILENAME=%DLURL_LIST_NAME%.conf
ECHO CALL ytdlp-dl.cmd %%BATCH_LIST_DIR%%\%CONFIG_FILENAME% %%BATCH_LIST_DIR%%\%YTDLP_DLURL_LIST_NAME%>>%BATCH_FILE%
REM ECHO REM yt-dlp.exe --config-location %%BATCH_LIST_DIR%%\%CONFIG_FILENAME% %%YTDLP_BATCH_OPT%% %%YTDLP_OPT%% -a %%BATCH_LIST_DIR%%\%YTDLP_DLURL_LIST_NAME%>>%BATCH_FILE%
...
SET YTDLP_CONF_OPT=--config-location %YTDLP_PROFILE_FILE%
