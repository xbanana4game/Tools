...
SET YTDLP_DLURL_LIST=%DLURL_LIST%.%END_FILE_EXT%
...
CD /D %VIDEOS_DIR%
yt-dlp.exe %YTDLP_CONF_OPT% %YTDLP_RESOLUTION_OPT% -a %DLURL_LIST_OUTPUT%.%END_FILE_EXT% 2>%YTDLP_ERR_FILE%
IF %ERRORLEVEL% EQU 0 (
	REM NO ERROR
	DEL %YTDLP_ERR_FILE%
)
...
IF NOT EXIST %YTDLP_DLURL_LIST% EXIT /B

SET YTDLP_CONF_OPT=--config-location %YTDLP_CONFIG_FILE%
SET YTDLP_ERR_FILE=%VIDEOS_DIR%\%DLURL_LIST_NAME%_%yyyy%%mm%%dd%%hh%%mn%%ss%.err
SET YTDLP_LOCK_FILE=%DESKTOP_DIR%\%DLURL_LIST_NAME%.lock
ECHO BATCH_MODE: %DLURL_LIST_NAME%
IF EXIST %YTDLP_LOCK_FILE% EXIT /B
IF EXIST %DESKTOP_DIR%\%DLURL_LIST_NAME%*.err EXIT /B
ECHO;>%YTDLP_LOCK_FILE%
REM CALL :FIND_HISTORY %YTDLP_DLURL_LIST%
IF %SKIP_FLG%==1 (
	strip_url.py %YTDLP_DLURL_LIST%
	IF ERRORLEVEL 1 (
		DEL url.txt
		PAUSE
		EXIT
	) ElSE (
		MOVE url.txt %YTDLP_DLURL_LIST%
	)
	type nul>%YTDLP_DLURL_LIST%.tmp
	FOR /F "tokens=1,2 delims=," %%i IN (%YTDLP_DLURL_LIST%) DO (
		FINDSTR /C:"%%j" %DLURL_HISTORY_FILES% >nul 2>&1
		IF ERRORLEVEL 1 (
			ECHO Start:%%i
			REM ECHO #Start DL: %%i>>%YTDLP_CONFIG_FILE%
			ECHO %%i>>%YTDLP_DLURL_LIST%.tmp
		) ELSE (
			ECHO Skip:%%i
			ECHO #Skip:%%i>>%YTDLP_CONFIG_FILE%
		)
	)
	IF EXIST %YTDLP_DLURL_LIST%.tmp MOVE %YTDLP_DLURL_LIST%.tmp %YTDLP_DLURL_LIST%
)
