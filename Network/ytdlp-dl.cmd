...
IF NOT DEFINED DLURL_LIST_NAME SET DLURL_LIST_NAME=_%yyyy%%mm%%dd%_%~n1_%hh%%mn%%ss%
...
SET BATCH_ERR_FILE=%DESKTOP_DIR%\%DLURL_LIST_NAME%.err
ECHO IF EXIST %BATCH_ERR_FILE% GOTO :%YTDLP_PROFILE%_%yyyy%%mm%%dd%-%hh%%mn%%ss%_END>>%BATCH_FILE%
...
REM ECHO CALL %TOOLS_DIR%\Network\ytdlp-dl.cmd %YTDLP_CONFIG_FILE% %DLURL_LIST_OUTPUT%.%END_FILE_EXT%>>%BATCH_FILE%
ECHO CALL %TOOLS_DIR%\Network\ytdlp-dl.cmd %%BATCH_LIST_DIR%%\%DLURL_LIST_NAME%.dlp %%BATCH_LIST_DIR%%\%DLURL_LIST_NAME%.txt.%END_FILE_EXT%>>%BATCH_FILE%
...
:DL_START_BATCH
SET BATCH_ERR_FILE=%DESKTOP_DIR%\%~n1.err
