...
SET YT_DLP_ERR_FILE=%VIDEOS_DIR%\%YT_DLP_ERR_FILE_NAME%
...
:DL_START_FILE
IF EXIST %YTDLP_DL_LIST_FILE% COPY %YTDLP_DL_LIST_FILE% %DLURL_LIST_OUTPUT%
IF NOT EXIST %DLURL_LIST_OUTPUT% TYPE nul >%DLURL_LIST_OUTPUT%
SET BATCH_LOCK_FILE=%DESKTOP_DIR%\%DLURL_LIST_NAME%.lock
NOTEPAD %DLURL_LIST_OUTPUT%
...
IF %SKIP_FLG%==1 (
	strip_url.py %DLURL_LIST_OUTPUT%
	IF ERRORLEVEL 1 (
		DEL url.txt
		PAUSE
		EXIT
...
CD /D %VIDEOS_DIR%
COPY %DLURL_LIST_OUTPUT% %DLURL_LIST_OUTPUT%.%END_FILE_EXT%
...
IF ERRORLEVEL 1 (
	...
	REM ERROR
	FINDSTR /C:"ERROR" %YT_DLP_ERR_FILE%>%DESKTOP_DIR%\%YT_DLP_ERR_FILE_NAME%
	REM MOVE %YT_DLP_ERR_FILE% %DESKTOP_DIR%\
)
EXIT /B
