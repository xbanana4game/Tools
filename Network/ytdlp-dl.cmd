...
SET YTDLP_CONFIG_FILE_DEFAULT=%VIDEOS_DIR%\%~n0.conf
IF NOT DEFINED YTDLP_CONFIG_FILE_DEFAULT SET YTDLP_CONFIG_FILE_DEFAULT=%VIDEOS_DIR%\yt-dlp.conf
IF NOT DEFINED YTDLP_CONFIG_FILE SET YTDLP_CONFIG_FILE=%VIDEO_OUTPUT_DIR%\yt-dlp_%yyyy%%mm%%dd%-%hh%%mn%%ss%.dlp
IF NOT DEFINED YTDLP_DL_LIST_FILE SET YTDLP_DL_LIST_FILE=%DESKTOP_DIR%\dl.txt
IF NOT DEFINED DLURL_LIST SET DLURL_LIST=%VIDEO_OUTPUT_DIR%\yt-dlp_%yyyy%%mm%%dd%-%hh%%mn%%ss%
...
:SET_INPUT_MODE
CHOICE /C 12 /M "1:INPUT 2:FILE"
IF %ERRORLEVEL% EQU 1 (
	SET MODE=INPUT
) ELSE IF %ERRORLEVEL% EQU 2 (
	SET MODE=FILE
	SET /P YTDLP_DL_LIST_FILE="input txt: "
)
SET YTDLP_DL_LIST_FILE=%YTDLP_DL_LIST_FILE:"=%
SET FILE_EXT=%YTDLP_DL_LIST_FILE:~-7,-4%
IF "%FILE_EXT%"=="csv" (
...
IF EXIST %DLURL_LIST%.log (
	MOVE %DLURL_LIST%.log %YT_DLP_LOG_DIR%\
	MOVE %YTDLP_CONFIG_FILE% %YT_DLP_LOG_DIR%\
)
IF EXIST %DLURL_LIST%.log DEL %DLURL_LIST%.log
IF EXIST %YTDLP_CONFIG_FILE% DEL %YTDLP_CONFIG_FILE%
